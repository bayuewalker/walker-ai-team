...
- final review path is governed by Validation Tier and Claim Level
...
```text
COMMANDER → FORGE-X → (Codex code review for MINOR and STANDARD) → (SENTINEL for MAJOR only) → (BRIEFER if requested/required) → COMMANDER
```

Rules:
- FORGE-X always comes first for build tasks
- Codex code review is required for MINOR tasks
- Codex code review is also required baseline review for STANDARD tasks
...
- Codex code review = REQUIRED
- FORGE-X self-check + Codex code review + COMMANDER review is sufficient
- MINOR tasks do not go to SENTINEL
...
- SENTINEL = NOT REQUIRED
- Codex code review = REQUIRED
- COMMANDER review decides merge / hold / rework after Codex review
- STANDARD tasks do not go to SENTINEL
...
Hard policy:
- SENTINEL is used only for MAJOR tasks
- MINOR and STANDARD tasks must stay on Codex code review + COMMANDER review path
- If COMMANDER wants SENTINEL, the task must first be reclassified to MAJOR
...
- MINOR tasks require Codex code review + COMMANDER review before merge
- STANDARD tasks require Codex code review + COMMANDER review before merge
...
```text
Codex code review required before merge. COMMANDER review after code review. Source: [forge report path]. Tier: STANDARD
```

If Validation Tier = MINOR:
write in `NEXT PRIORITY` one of:
- `Codex code review required before merge. COMMANDER review after code review. Source: [forge report path]. Tier: MINOR`
- or
...
### CODEX CODE REVIEW (MINOR / STANDARD)

Codex code review is the default review layer for MINOR tasks and the baseline review layer for STANDARD tasks.

Purpose:
- catch simple regressions early
- reduce unnecessary SENTINEL workload
- keep small changes fast and deterministic
- give COMMANDER a lightweight review artifact before merge decision
...
Use this for MINOR tasks and STANDARD tasks.
...
Required checks:
1. report exists at exact forge path
2. report naming/path valid
3. report contains all 6 mandatory sections
4. PROJECT_STATE.md updated if repo changed
5. final output includes:
   - Report: [full forge report path]
   - State: PROJECT_STATE.md updated
   - Validation Tier: [MINOR/STANDARD]
   - Claim Level: [FOUNDATION / NARROW INTEGRATION / FULL RUNTIME INTEGRATION]
6. declared Claim Level matches the actual implementation
7. no forbidden `phase*/` folders
8. no legacy imports introduced
9. no hardcoded secrets
10. no accidental risk-rule drift
11. changed code is internally consistent and matches task scope
...
Next:
- COMMANDER review
- or SENTINEL escalation if review reveals major-risk impact
```
...
- FOUNDATION claim defaults to CONTRACT_CHECK only when the task is MAJOR
- NARROW INTEGRATION claim defaults to NARROW_INTEGRATION_CHECK only when the task is MAJOR
- FULL RUNTIME INTEGRATION claim defaults to FULL_RUNTIME_AUDIT when the task is MAJOR
- POST_MERGE_AUDIT is MAJOR-only and must be explicitly named by COMMANDER
...
COMMANDER → decides validation path by tier
    ↓
if MINOR:
    Codex code review → COMMANDER review → merge decision
    ↓
if STANDARD:
    Codex code review → COMMANDER review → merge / hold / rework decision
    ↓
...
- COMMANDER decides
- MINOR tasks use Codex code review by default
- STANDARD tasks use Codex code review first, then COMMANDER decides merge / hold / rework
...
- MINOR or STANDARD task was unnecessarily escalated to SENTINEL despite major-only validation policy
...
- MINOR task merged without Codex code review
