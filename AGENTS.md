...
- Branch names themselves must follow the authoritative branch naming format defined in the **BRANCH NAMING** section (`feature/{feature}` with a hyphen‑separated slug).  Using dots (`.`), underscores (`_`), or other separators inside phase tokens (e.g., `11.1`) is **invalid** and constitutes drift.
- If the current working or PR head branch uses an invalid format (such as `phase11.1`), **STOP** and rename the branch to a compliant hyphen form (e.g., `phase11-1`) before continuing.
- Reports and state files must never convert, abbreviate, or reformat the branch name.  They must mirror the exact branch string from Git.
...
- final output claims Done but missing explicit `State:` confirmation
 - branch mismatch or invalid branch format is a workflow‑blocking defect. Branch names must follow the authoritative `feature/{feature}` hyphenated pattern with no dots or underscores. Stop and rename the branch if it deviates, and update all reports to reflect the exact branch string before proceeding.
...
- branch mismatch or invalid branch format is a workflow-blocking defect.  If the branch name in Git does not exactly match the name in the report or uses an invalid format (such as dots in phase tokens), STOP, fix the branch name and update all affected artifacts before continuing.
