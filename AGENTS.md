...
```text
COMMANDER → FORGE-X → (Codex code review for MINOR and STANDARD) → (SENTINEL if REQUIRED or explicitly requested) → (BRIEFER if requested/required) → COMMANDER
```

Rules:
- FORGE-X always comes first for build tasks
- Codex code review is required for MINOR tasks
- Codex code review is also recommended baseline review for STANDARD tasks
- SENTINEL is mandatory only for MAJOR tasks or explicit COMMANDER request
...
- SENTINEL may still run only if COMMANDER explicitly requests it
...
Rule:
- SENTINEL = RECOMMENDED, not automatic
- Codex code review = REQUIRED
- COMMANDER decides based on regression risk, user visibility, and release importance
...
- STANDARD tasks require Codex code review and may require SENTINEL if COMMANDER requests or escalates them
...
- `Codex code review required. COMMANDER review for validation decision. Source: [forge report path]. Tier: STANDARD`
- or, if COMMANDER already requested validation:
  `SENTINEL validation requested for [task name]. Source: [forge report path]. Tier: STANDARD`
...
Rule:
- FORGE-X must not auto-force SENTINEL for every task
...
Use this for MINOR tasks, or for STANDARD tasks before deciding whether SENTINEL is needed.
...
6. no forbidden `phase*/` folders
7. no legacy imports introduced
8. no hardcoded secrets
9. no accidental risk-rule drift
10. changed code is internally consistent and matches task scope
...
5. Read the declared Validation Tier and Validation Target from the forge report
...
if MINOR:
    ...
    ↓
if STANDARD:
    Codex code review → COMMANDER decides hold / merge / escalate to SENTINEL
    ↓
...
- STANDARD tasks use Codex code review first, then COMMANDER decides escalation
...
- SENTINEL ran without checking declared Validation Tier and Validation Target first
